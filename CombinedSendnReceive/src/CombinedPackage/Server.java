		...
		Thread dataReaderThread = new Thread(() -> {
			try (ServerSocket serverSocket = new ServerSocket(12345)) {
				while (true) {
					...
					clientName.setText("Receiving from : "
							+ clientAddress.toString().substring(1, clientAddress.toString().length()) + " ("
							+ clientAddress.getHostName() + " )");

					int handShake = dataInputStream.readInt();
					if (handShake == 1111) {
						try (Socket sockett = new Socket(clientAddress, 1235)) {
							DataOutputStream dataOutputStream = new DataOutputStream(sockett.getOutputStream());
							dataOutputStream.writeInt(2222);
							ObjectOutputStream oos = new ObjectOutputStream(sockett.getOutputStream());
							oos.writeObject(publicKey);
						} catch (IOException ioe) {
							// ioe.printStackTrace();
							// System.out.println("Client is not online to accept key");
							JOptionPane.showMessageDialog(null, "Client is not online to accept key", "Message",
			...
			try (ServerSocket serverSocket = new ServerSocket(1234)) {
				while (true) {
					...
					if (fNameLength > 0) {
						...
						jplRows.setBorder(BorderFactory.createCompoundBorder(new LineBorder(new Color(0x0D6E6E)),
						...
						myFiles.add(new MyFile(fileId, fName, decryptedData, getFileExtension(fName), unDecryptedData));
						fileId++;
					}
				}
	...
	public MouseListener getMyMouseListener() {
		return new MouseListener() {
			...
			public void mouseClicked(MouseEvent me) {
				...
				if (source instanceof JButton) {
					...
					if (clickedButton.getText() == "View") {
						...
						for (MyFile myFile : myFiles) {
							if (myFile.getId() == fileId) {
								JFrame jPreviewFrame = createFrame(myFile.getName(), myFile.getUnDecryptedData(),
					...
					if (clickedButton.getText() == "View Decrypted") {
						...
						for (MyFile myFile : myFiles) {
							if (myFile.getId() == fileId) {
								JFrame jPreviewFrame = createFrame(myFile.getName(), myFile.getData(),
										myFile.getFileExtension());
			...
			private JFrame createFrame(String fileName, byte[] fileData, String fileExtension) {
				...
				JLabel jPrompt = new JLabel("<html>Are you sure you want to download <br>" + fileName + "</html>");
				...
				yesBtn.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent ae) {
						...
						try {
							...
							JButton fileExplorerBtnButton = new JButton("<html>View in <br>File Explorer</html>");
							...
							fileExplorerBtnButton.addActionListener(new ActionListener() {
								@Override
